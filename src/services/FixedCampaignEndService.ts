...
class FixedCampaignEndService {
  ...
  private async generateNFTFromIPFS(rarity: NFTRarity, project: NFTProject): Promise<NFTData | null> {
    try {
      // Get NFT data from IPFS based on rarity
      const nftsByRarity = await ipfsService.getNFTsByRarity(rarity);
      
      if (!nftsByRarity || nftsByRarity.length === 0) {
        console.warn(`No ${rarity} NFTs found in IPFS`);
        return null;
      }

      // Select a random NFT from the rarity pool
      const randomIndex = Math.floor(Math.random() * nftsByRarity.length);
      const selectedNFT = nftsByRarity[randomIndex];

      // Create NFT data with project context
      const nftData: NFTData = {
        id: `${project.id}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        name: selectedNFT.name || `${project.projectName} ${rarity}`,
        description: selectedNFT.description || `${rarity} NFT from ${project.projectName} campaign`,
        image: selectedNFT.image || project.image,
        rarity: rarity,
        attributes: selectedNFT.attributes || [
          { trait_type: "Campaign", value: project.projectName },
          { trait_type: "Rarity", value: rarity },
          { trait_type: "Collection", value: project.nftName }
        ],
        wallet_address: '', // Will be set when distributing
        claimed: false,
        created_at: new Date().toISOString(),
        project_id: project.id,
        collection_name: project.nftName
      };

      return nftData;
    } catch (error) {
      console.error(`Error generating ${rarity} NFT from IPFS:`, error);
      return null;
    }
  }
...
const fixedCampaignEndService = new FixedCampaignEndService();
export default fixedCampaignEndService;
