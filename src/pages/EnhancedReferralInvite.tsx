...
import { Gift, Users, Sparkles, CheckCircle, Clock } from 'lucide-react';
import { referralService } from '@/services/ReferralService';
import { enhancedReferralService } from '@/services/EnhancedReferralService';
import { useWallet } from '@/components/wallet/WalletProvider';
import { useEnhancedReferral } from '@/hooks/useEnhancedReferral';
import { toast } from 'sonner';
import { useAuthState } from '@/hooks/useAuthState';

const EnhancedReferralInvite: React.FC = () => {
  const { referralCode } = useParams<{ referralCode: string }>();
  const navigate = useNavigate();
  const { isAuthenticated, walletAddress } = useAuthState();
  const { processReferral, eligibility, checkEligibility } = useEnhancedReferral();
  ...
  }, [isAuthenticated, walletAddress, checkEligibility]);

  // Attempt to process referral when user becomes eligible
  useEffect(() => {
    const attemptReferralProcessing = async () => {
      if (!isAuthenticated || !walletAddress || isProcessing || !eligibility.eligible) return;
      ...
      if (pendingCode && pendingReferrer && pendingCode === referralCode) {
        ...
        try {
          console.log('🎯 Processing referral for eligible user:', { pendingReferrer, walletAddress });
          const success = await processReferral(pendingReferrer);
          
          if (success) {
            toast.success('🎉 Congratulations! Your referral reward has been processed!');
    ...
    attemptReferralProcessing();
  }, [isAuthenticated, walletAddress, referralCode, processReferral, navigate, isProcessing, eligibility.eligible]);
    ...
    <div className="min-h-screen bg-[#0F1114] flex items-center justify-center p-4">
      <div className="max-w-md mx-auto text-center">
        <div className="bg-[#121021] border border-white/10 rounded-2xl p-8 shadow-xl">
          ...
          <p className="text-gray-400 mb-6">
            Join NEFTIT and complete 2 projects to earn rewards together
          </p>
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4 mb-6">
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              <span className="text-sm text-gray-300">Referral Requirements</span>
            </div>
            <p className="text-yellow-500 text-sm">
              Complete 2 projects (100%) to activate referral rewards
            </p>
          </div>

          {/* Progress Display */}
          {isAuthenticated && (
            <div className="bg-[#1a1a2e]/50 border border-white/10 rounded-lg p-4 mb-6">
              ...
              <div className="flex items-center justify-center gap-4">
                <div className="text-center">
                  <p className="text-2xl font-bold text-[#5d43ef]">
                    {eligibility.completed_projects}
                  </p>
                ...
                <div className="text-center">
                  <p className="text-2xl font-bold text-gray-400">
                    {eligibility.required_projects}
                  </p>
                  <p className="text-xs text-gray-400">Required</p>
                </div>
              </div>
              {eligibility.remaining_projects > 0 && (
                <p className="text-sm text-orange-400 mt-2">
                  Complete {eligibility.remaining_projects} more project{eligibility.remaining_projects > 1 ? 's' : ''} to activate referral
                </p>
              )}
          ...
          {!isAuthenticated ? (
            <div>
              ...
              >
                Login & Start Completing Projects
              </Button>
            </div>
          ) : isProcessing ? (
            <div className="text-center">
            ...
            <div>
              <p className="text-sm text-green-400 mb-4">
                ✅ Requirements met! Processing your referral...
              </p>
              <Button 
...
export default EnhancedReferralInvite;
