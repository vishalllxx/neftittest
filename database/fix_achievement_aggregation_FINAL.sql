-- ============================================================================
-- FIX ACHIEVEMENT AGGREGATION - FINAL CORRECT VERSION
-- Fixes get_user_complete_balance to properly aggregate achievement rewards
-- by joining user_achievements with achievements_master table
-- ============================================================================

-- CORRECTED get_user_complete_balance function that joins with achievements_master
...
BEGIN
  ...
  -- CORRECTED: Get achievement rewards by joining with achievements_master
  SELECT 
...
-- CORRECTED claim_achievement_reward function (no manual balance updates needed)
...
BEGIN
  ...
  -- CORRECTED: Only mark as claimed - aggregation function handles the rest
  UPDATE user_achievements 
  SET claimed_at = NOW(), updated_at = NOW()
  WHERE wallet_address = user_wallet AND achievement_key = achievement_key_param;
  
  -- Trigger balance sync to update user_balances table with aggregated data
  PERFORM sync_user_balance_enhanced(user_wallet);
...
SELECT 'FINAL ACHIEVEMENT AGGREGATION FIX DEPLOYED!' as status;
SELECT 'Achievement rewards now properly aggregated from achievements_master table!' as info;
SELECT 'No need for reward columns in user_achievements table!' as details;
