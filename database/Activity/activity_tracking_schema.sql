-- Activity Tracking Schema for Supabase
-- Tracks user activities and interactions for wallet addresses
...
CREATE TABLE IF NOT EXISTS user_activities (
  ...
  nft_reward TEXT, -- NFT ID or type if applicable
...
-- Policy: Users can only see their own activity stats
...
-- Records a new activity and updates statistics
...
BEGIN
  ...
  INSERT INTO user_activity_stats (
    ...
    CASE WHEN activity_type_param = 'claim' THEN 1 ELSE 0 END,
...
-- 6. Function to Get User Activities
...
BEGIN
  RETURN QUERY
  SELECT 
    ua.id, ua.activity_type, ua.activity_title, ua.activity_description,
    ua.details, ua.neft_reward, ua.xp_reward, ua.nft_reward,
    ua.status, ua.metadata, ua.created_at
...
-- 7. Function to Get User Activity Statistics
...
BEGIN
  RETURN QUERY
  SELECT 
    COALESCE(uas.total_activities, 0),
    COALESCE(uas.total_tasks_completed, 0),
    COALESCE(uas.total_claims, 0),
    COALESCE(uas.total_burns, 0),
    COALESCE(uas.total_stakes, 0),
    COALESCE(uas.total_neft_earned, 0.0),
    COALESCE(uas.total_xp_earned, 0),
    uas.last_activity_at
  FROM user_activity_stats uas
  WHERE uas.wallet_address = user_wallet
  UNION ALL
  SELECT 0, 0, 0, 0, 0, 0.0::DECIMAL(18,8), 0, NULL::TIMESTAMP WITH TIME ZONE
  WHERE NOT EXISTS (SELECT 1 FROM user_activity_stats WHERE wallet_address = user_wallet)
  LIMIT 1;
END;
$$ LANGUAGE plpgsql;

-- 8. Trigger Function to Update Activity Stats
-- Automatically updates stats when activities are inserted
CREATE OR REPLACE FUNCTION update_activity_stats_trigger()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE INDEX IF NOT EXISTS idx_user_activities_wallet_type ON user_activities(wallet_address, activity_type);
CREATE INDEX IF NOT EXISTS idx_user_activity_stats_wallet ON user_activity_stats(wallet_address);

-- 10. Comments for Documentation
COMMENT ON TABLE user_activities IS 'Records all user activities and interactions';
COMMENT ON TABLE user_activity_stats IS 'Aggregated statistics for user activities';
COMMENT ON FUNCTION log_user_activity IS 'Logs a new user activity and updates statistics';
COMMENT ON FUNCTION get_user_activities IS 'Returns paginated user activities with optional filtering';
COMMENT ON FUNCTION get_user_activity_statistics IS 'Returns aggregated activity statistics for a user';
