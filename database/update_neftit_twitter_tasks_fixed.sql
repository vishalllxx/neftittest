-- Update NEFTIT Project Twitter Tasks - FIXED & OPTIMIZED FOR LOW EGRESS
-- Project ID: 441faa21-27a3-465c-a05b-bfd94d0259bc

-- Step 1: Check current project status (minimal query)
SELECT 
    p.id,
    p.title,
...
-- Step 2: Check existing tasks (minimal query)
SELECT 
    id,
    title,
    type,
...
-- Step 4: Insert working Twitter tasks (optimized for low egress)
...
-- Step 5: Verify tasks were created (minimal query)
SELECT 
    id,
    title,
    type,
...
-- Step 6: Update project metadata (minimal update)
UPDATE projects 
SET 
    updated_at = NOW(),
    task_status = 'active'
...
-- Step 7: Show final project status (compatible with older PostgreSQL)
SELECT 
    p.title,
    p.current_participants,
    p.max_participants,
...
-- Step 8: Ensure user_task_completions table exists (minimal structure)
...
-- Step 9: Create indexes for performance (minimal indexes)
...
-- Step 10: Final verification (compatible query)
SELECT 
...
-- Step 11: Show task types (compatible with older PostgreSQL)
SELECT 
    type,
    COUNT(*) as count
FROM project_tasks 
