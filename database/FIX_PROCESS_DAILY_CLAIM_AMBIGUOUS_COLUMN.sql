-- ============================================================================
-- FIX AMBIGUOUS COLUMN REFERENCE IN PROCESS_DAILY_CLAIM
-- ============================================================================
-- Fixes the "column reference 'total_neft_earned' is ambiguous" error
-- by using table aliases to disambiguate column references
-- ============================================================================

-- Drop and recreate the process_daily_claim function with proper column disambiguation
...
BEGIN
  ...
  -- Update user streaks with 24-hour timestamp (use table alias to avoid ambiguity)
  ...
  -- Update user_balances table (use table alias to avoid ambiguity)
...
DECLARE
  test_result RECORD;
BEGIN
  -- Test with social login wallet (should fail due to cooldown but function should work)
  SELECT * INTO test_result FROM process_daily_claim('social:google:108350092537307288909');
  RAISE NOTICE 'process_daily_claim function fixed successfully. Test result: success=%, message=%', 
...
SELECT 'process_daily_claim function fixed - ambiguous column references resolved!' as status,
       'Used table aliases to disambiguate total_neft_earned and total_xp_earned columns' as result;
