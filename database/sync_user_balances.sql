...
SELECT 'Checking synced balance...' as step;
SELECT 
  wallet_address,
  total_neft_claimed,
  total_xp_earned,
...
-- Test the aggregation function to compare
SELECT 'Comparing with aggregation function...' as step;
SELECT get_user_complete_balance('social:google:108350092537307288909');

-- Sync all existing users (optional - run if you want to sync all users)
-- SELECT 'Syncing all user balances...' as step;
-- SELECT sync_all_user_balances();

SELECT 'Balance sync system created successfully!' as status;
SELECT 'user_balances table will now automatically stay in sync with all reward sources!' as info;
